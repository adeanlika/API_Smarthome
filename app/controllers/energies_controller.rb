class EnergiesController < ApiController
  ...
  def get_daily(start_date)
    # koding versi baruww
    Time.zone = "Bangkok"
    @count = Energy.joins(:home).where('homes.id = ?',params[:home_id]).group_by_day('energies.created_at', range: start_date.in_time_zone("Jakarta")..(start_date + 1.month).in_time_zone("Bangkok")).count(:total)
    @count = @count.collect {|ind| ind[1]}
  ...
  def get_monthly(start_date)
    ...
    @energy = Energy.joins(:home).where('homes.id = ?', params[:home_id]).select("total,energies.created_at").order('created_at ASC')
    @energy = @energy.where('energies.created_at' => start_date.in_time_zone("Bangkok")..(start_date + 1.year).in_time_zone("Bangkok")).group_by {|t| t.created_at.beginning_of_month}
    if not @energy.empty?
    # @energy_first = @energy.collect { |t, d|   { t => d.first[:total] } }.first.values
    @energy_last =  @energy.collect { |t, d|   { t => d.last[:total] } }
    @energy_last = @energy_last.map{|x| x.values}.collect {|ind| ind[0]}
  ...
  def get_yearly(start_date)
    Time.zone = "Bangkok"
    @count = Energy.joins(:home).where('homes.id = ?',params[:home_id]).group_by_year('energies.created_at', range: (start_date - 6.year).in_time_zone("Bangkok").. (start_date + 6.year - 1.day)).count(:total)
    @count = @count.collect {|ind| ind[1]}

    @energy = Energy.joins(:home).where('homes.id = ?', params[:home_id]).select("total,energies.created_at").order('created_at ASC')
    @energy = @energy.where('energies.created_at' => (start_date - 6.year).in_time_zone("Bangkok")..(start_date + 6.year).in_time_zone("Bangkok")).group_by {|t| t.created_at.beginning_of_year}
    if not @energy.empty?
    @energy_first = @energy.collect { |t, d|   { t => d.first[:total] } }.first.values
    @energy_last =  @energy.collect { |t, d|   { t => d.last[:total] } }
    @energy_last = @energy_last.map{|x| x.values}.collect {|ind| ind[0]}
   ...
   @count.each_index do |index|
     if @count[index] != 0
       if first == index
         @energy_first = Energy.where('created_at < ? AND home_id = ?',(start_date -  6.year).in_time_zone("Bangkok"),params[:home_id]).order('created_at ASC').select('total').last
         if @energy_first.present?
           @yearly_bar << @energy_last[0] - @energy_first.total
         else
           @energy_first = @energy.collect { |t, d|   { t => d.first[:total] } }.first.values
