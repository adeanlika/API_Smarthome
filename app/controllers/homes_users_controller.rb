class HomesUsersController < ApiController
  ...
  def home_member
    @member = User.joins(:homes_users).where('homes_users.home_id': params[:home_id]).select('users.id',:email,:image,'homes_users.is_admin')
    render json: @member
  end
  ...
  def add_member
    ...
    if @new_member.nil?
      ...
      @new_member = 'this user is already a member'
    else
